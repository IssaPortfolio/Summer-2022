...
int main(int argc, char *argv[])  // argc is number of arguments and argv is the "string" of the arguments or a list of char data type
{
    ...
    int a_count = 0, e_count = 0, i_count = 0, o_count = 0, u_count = 0, z_count = 0, y_count = 0, k_count = 0; 
    ...
    while (!InputStream.eof() && InputStream >> extractCharacter) 
    {
        ...
        if (extractCharacter == 'A' || extractCharacter == 'a')
            ...
            y_count += 1;
        else if (extractCharacter == 'K' || extractCharacter == 'k')
            k_count += 1;
    }
    ...
    cout << right << setw(61) << left << NUM_OF_SENTENCE + "K's: " << k_count << endl;
    //  Total of all vowels
    const int VOWEL_TOTAL = a_count + e_count + i_count + o_count + u_count + z_count + y_count + k_count;
